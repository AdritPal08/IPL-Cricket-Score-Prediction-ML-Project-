...
class LoadArtifacts:
    ...
    def load_artifacts(self):
        ...
        return list(encoded_teams.keys()), columns[6:].tolist() + ["Abu Dhabi, UAE"]

    # def predict_score(self, batting_team, bowling_team, innings, over, runs, wickets, venue, columns):
    def predict_score(self, batting_team:str, bowling_team:str, innings:int, over:int, runs:int, wickets:int, venue:str):
        ...
        try:
            ...
            if venue != "Abu Dhabi, UAE":
                venue_index = np.where(columns == venue)[0][0]+1  # add 1 to skip the first column
